...
const BLOG = {
  ...
  GREETING_WORDS: process.env.NEXT_PUBLIC_GREETING_WORDS || '说点什么呢？🎉',

  CUSTOM_MENU: process.env.NEXT_PUBLIC_CUSTOM_MENU || true, // 支持Menu 类型，从3.12.0版本起，各主题将逐步支持灵活的二级菜单配置，替代了原来的Page类型，此配置是试验功能、默认关闭。

  AUTHOR: process.env.NEXT_PUBLIC_AUTHOR || '有闲', // 您的昵称 例如 tangly1024
  BIO: process.env.NEXT_PUBLIC_BIO || '晴，无事。', // 作者简介
  ...
  FONT_URL: [
    ...
    'https://cdn.jsdelivr.net/npm/lxgw-wenkai-screen-webfont@1.1.0/style.css', //霞鹜文楷屏幕阅读版
  ],
  ...
  NOTION_PROPERTY_NAME: {
    ...
    type: process.env.NEXT_PUBLIC_NOTION_PROPERTY_TYPE || '类型', // 文章类型，默认为type
    type_post: process.env.NEXT_PUBLIC_NOTION_PROPERTY_TYPE_POST || '文', // 当type文章类型与此值相同时，为博文，默认为Post。
    type_page: process.env.NEXT_PUBLIC_NOTION_PROPERTY_TYPE_PAGE || '页', // 当type文章类型与此值相同时，为单页，默认为Page。
    type_notice:
    ...
    title: process.env.NEXT_PUBLIC_NOTION_PROPERTY_TITLE || '标题', // 文章标题，默认为title
    status: process.env.NEXT_PUBLIC_NOTION_PROPERTY_STATUS || '状态', //默认为status
    status_publish:
        process.env.NEXT_PUBLIC_NOTION_PROPERTY_STATUS_PUBLISH || '发', // 当status状态值与此相同时为发布，可以为中文，默认为Published。
    status_invisible:
        process.env.NEXT_PUBLIC_NOTION_PROPERTY_STATUS_INVISIBLE || '藏', // 当status状态值与此相同时为隐藏发布，可以为中文， 除此之外其他页面状态不会显示在博客上，默认为Invisible。
    summary: process.env.NEXT_PUBLIC_NOTION_PROPERTY_SUMMARY || '摘要', // 默认为summary。
    slug: process.env.NEXT_PUBLIC_NOTION_PROPERTY_SLUG || '链接', //默认为slug
    category: process.env.NEXT_PUBLIC_NOTION_PROPERTY_CATEGORY || '分类', //默认为category
    date: process.env.NEXT_PUBLIC_NOTION_PROPERTY_DATE || '日期', //默认为date
    tags: process.env.NEXT_PUBLIC_NOTION_PROPERTY_TAGS || '标签', //默认为tags
    icon: process.env.NEXT_PUBLIC_NOTION_PROPERTY_ICON || '图标' //默认为icon
  },
